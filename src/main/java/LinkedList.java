...
//    }
     ...
     * everybody know that out head value has changed. So the work around for
     * his is rather than giving everyone an access to the head pointer directly
    ...
    public class LinkedList {
        ...
        public void deleteWithValue(int data){
            if(head == null) return; //nothing can be done here
            ...
            //warning: what if the node we want to delete is the head node.
            //The head node is never explicitly "destroyed" but we cannot access
            //it so it is essentially gone.
            if(head.data == data){
                head = head.next; //cut it out of the LL
                return;
            }
            ...
            while(current.next != null){
                if(current.next.data == data){ // if the next value is the one we want to delete
